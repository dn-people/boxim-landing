...
브랜드는 동네방네(`https://dnbn.co.kr/`)이며, 독자에게 검증된 통신·IT·가전 렌탈 정보를
제공하는 것이 우선입니다.
...
6. `categoryDiversity.nextCategory`와 `docs/blog/CATEGORY_STRATEGY.md`를 확인하고, 선택한 카테고리와
   우선순위를 벗어난 이유가 있다면 발행 노트에 기록한다.

## 1. 이슈 탐색과 주제 선정

모든 글은 아래 5개 카테고리 중 하나만 선택한다. 저장값은 괄호 안 ID를 사용한다.

- **IT·테크 (`it-tech`):** 기기 기능, 운영체제, 앱, 보안, 수리·관리
- **통신사 이슈 (`carrier-issues`):** 통신사 공지, 청구·결제·개통 문제, 제도 변화와 대응
- **렌탈 (`rental`):** 가전 렌탈의 계약, 월요금, 설치, 관리, 반환, 해지
- **제품 리뷰 (`product-reviews`):** 특정 제품의 공식 사양 비교와 사용 목적별 선택
- **구매·요금 가이드 (`buying-guides`):** 스마트폰·요금제·인터넷 구매, 비용, 약정, 비교

최근 10편에서 5개 카테고리를 각 2편씩 확보하는 것을 기준으로 삼는다. 기본 예약 발행은
`categoryDiversity.nextCategory`의 후보를 먼저 검토하고 같은 카테고리를 3편 연속 발행하지 않는다.
공식 발표가 나온 긴급 통신사 이슈는 순환보다 우선할 수 있지만, PR에 발표 시각과 예외 이유를 남긴다.
...
렌탈 글은 동네방네가 통신판매중개자이며 계약 당사자는 각 렌탈 제휴사라는 역할을 분명히 적고,
계약기간·의무사용기간·소유권·해지비용·관리 주기를 상품별 공식 계약 조건으로 확인한다. 제품 리뷰는
실사용하지 않았다면 `사양 기반 리뷰`라고 밝히고, 써 본 것처럼 느껴지는 체험·성능·만족도 표현을 쓰지 않는다.
...
- **카테고리:** `dnbn:category` 메타와 Article JSON-LD의 `articleSection`을 선택한 카테고리와
  일치시키고, 목록 카드의 `data-category`와 `TOPICS.md`에도 같은 ID를 기록한다.
...
5. `docs/blog/TOPICS.md` 발행 표에 카테고리를 포함한 한 줄을 추가하고 소진한 백로그를 제거한다.
   해당 카테고리 후보가 2개 미만이 되면 검증된 후보를 보충한다.
